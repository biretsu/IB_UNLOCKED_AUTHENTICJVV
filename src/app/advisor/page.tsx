...
import { Bot, User, Sparkles, Loader2, SendHorizontal, Trash2, AlertCircle, Info } from "lucide-react";
...
export default function AdvisorPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errorMessage = error.message || "I encountered an error connecting to my core brain.";
      
      toast({
        variant: "destructive",
        title: "Advisor Unavailable",
        description: "There was a problem reaching the AI server. Please verify your environment configuration.",
      });
      
      setMessages(prev => [...prev, { 
        role: "bot", 
        content: `I apologize, but I encountered a system error: "${errorMessage}". If you are the administrator, please ensure the GOOGLE_GENAI_API_KEY is correctly set in your Vercel Environment Variables.`,
        isError: true
      }]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl h-[calc(100svh-4rem)] flex flex-col">
      <div className="flex flex-col flex-1 h-full space-y-4">
        ...
        <Alert variant="default" className="bg-primary/5 border-primary/20 text-primary mb-2">
          <Info className="h-4 w-4 text-accent" />
          <AlertDescription className="text-xs font-medium">
            <strong>Disclaimer:</strong> This AI provides data-driven suggestions. However, requirements change frequently. You <strong>must</strong> consult with your professional University Counselor and official university portals before finalizing subject choices.
          </AlertDescription>
        </Alert>
                  ...
                  <div key={i} className={cn(
                    ...
                    <div className={cn(
                      ...
                      msg.role === "user" 
                        ...
                        : msg.isError 
                          ? "bg-destructive/10 text-destructive border-destructive/20 border-2 rounded-tl-none"
                      ...
                      <div className={cn(
                        ...
                        msg.role === "user" ? "text-primary-foreground" : msg.isError ? "text-destructive" : "text-primary"
                      )}>
                        {msg.role === "user" ? <User className="w-3 h-3" /> : <Bot className="w-3 h-3" />}
                        {msg.role === "user" ? "Student" : "Advisor"}
                      </div>
                      <div className="whitespace-pre-wrap">{msg.content}</div>
                      {msg.isError && <AlertCircle className="absolute -right-6 top-0 w-4 h-4 text-destructive" />}
                    </div>
                  </div>
                ))}
                {isLoading && (
