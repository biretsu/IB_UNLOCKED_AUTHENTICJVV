...
import { CAREERS, IB_SUBJECTS, UNIVERSITIES, Career, Subject, University, CareerSubjectRequirement } from "@/app/lib/ib-data";
...
function PlannedFlowContent() {
  ...
  const groupRequirements = (reqs: CareerSubjectRequirement[]) => {
    const slots: { group?: string; requirements: CareerSubjectRequirement[] }[] = [];
    const seenGroups = new Set<string>();
    
    reqs.forEach(req => {
      if (req.group) {
        if (!seenGroups.has(req.group)) {
          seenGroups.add(req.group);
          slots.push({ 
            group: req.group, 
            requirements: reqs.filter(r => r.group === req.group) 
          });
        }
      } else {
        slots.push({ requirements: [req] });
      }
    });
    return slots;
  };
  ...
  const careerResults = useMemo(() => {
    const calculateMatch = (career: Career) => {
      ...
      const requiredSlots = groupRequirements(career.requiredSubjects);
      const recommendedSlots = groupRequirements(career.recommendedSubjects);
      const totalPossible = requiredSlots.length + recommendedSlots.length;

      const details: { label: string; isRequired: boolean; isMet: boolean }[] = [];

      requiredSlots.forEach(slot => {
        const isMet = slot.requirements.some(req => {
          const sel = selections.find(s => s.subjectId === req.id);
          return req.level === 'HL' ? sel?.level === 'HL' : !!sel;
        });
        if (isMet) score += 1;
        
        const label = slot.requirements.map(r => `${getSubjectName(r.id)} ${r.level}`).join(' or ');
        details.push({ label, isRequired: true, isMet });
      });
      ...
      recommendedSlots.forEach(slot => {
        ...
        const label = slot.requirements.map(r => `${getSubjectName(r.id)} ${r.level}`).join(' or ');
        details.push({ label, isRequired: false, isMet });
      });
  ...
  const getFilteredUnis = (careerId: string) => {
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {groupRequirements(selectedCareer.requiredSubjects).map((slot, idx) => (
                            <Badge key={`req-${idx}`} variant="default" className="bg-primary text-white rounded-full px-4 py-1.5 text-xs font-bold">
                              {slot.requirements.map(req => `${getSubjectName(req.id)} ${req.level}`).join(' or ')}
                            </Badge>
                          ))}
                        </div>
                      </div>
                      <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {groupRequirements(selectedCareer.recommendedSubjects).map((slot, idx) => (
                            <Badge key={`rec-${idx}`} variant="secondary" className="bg-muted text-primary rounded-full px-4 py-1.5 text-xs font-semibold">
                              {slot.requirements.map(req => `${getSubjectName(req.id)} ${req.level}`).join(' or ')}
                            </Badge>
                          ))}
                        </div>
            ...
            <div className="animate-fade-in space-y-10 py-8">
              ...
              <div className="space-y-12">
                {careerResults.map((career) => {
                  const careerUnis = getFilteredUnis(career.id);
                  return (
